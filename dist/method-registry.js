...
export const ANNOTATIONS = {
    ...
    modify: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
...
export function commonSendParams() {
    return [
...
export function commonMediaParams() {
    return [
...
export function commonEditParams() {
    return [
...
export function buildJsonSchema(params) {
    const properties = {};
    const required = [];
    for (const param of params) {
...
function zodToJsonSchema(schema, description) {
    const base = inferJsonType(schema);
    base.description = description;
    return base;
}
function inferJsonType(schema) {
    ...
    if (def.typeName === "ZodUnion") {
        const options = def.options.map((o) => inferJsonType(o));
